...
const formData = reactive({
  ...
  barangay_id: authStore.userRole === 'BARANGAY_STAFF' ? authStore.barangayId : null
});
...
onMounted(() => {
  ...
  const handleSubmit = async (status) => {
    ...
    if (success) {
      resetForm();
      const route = authStore.useRole === 'LGU_ADMIN' ? '/admin/announcements' : '/barangay/announcements'
      router.push(route); // redirect to announcements list
    }
  };
  </script>
